...
# Can't get this working on the file upload side, which is blocking a lot of testing here
Fabricator(:image) do
  translation { text }
  user_id { Fabricate(:user) }
  file { fixture_file_upload('files/tripta_iphone_4.JPG', 'image/jpg') }
  after_create { |img| img.stored_at = "https://s3.amazonaws.com/optifridge_images/image/#{self.id}/#{self.file}" }
end
